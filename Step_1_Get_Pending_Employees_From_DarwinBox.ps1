...
"@
        ...
        $api_URL = "https:///masterapi/employee"
...
Write-Log "Getting pending Employees from DarwinBox ..."

$Pending_EmployeesList = Get-AllEmployees -admin_Email $DWB_adminEmail -secretKey $DWB_secretKey `
                                           -UID $DWB_UID -datasetKey $DWB_active_dts_key

if ($Pending_EmployeesList.status -eq 1)
{
    
    $Pending_EmployeesList.employee_data | Export-CSV $DWB_PendingEmployees_output_CSV -NoTypeInformation -Encoding UTF8

    Write-Log "Employees exported successfully to CSV file"
}
else
{
    Write-Log "No users found in the list of Pending DarwinBox Employees." -Level Warn
}
